...
rollback() {
    ...
    # 安全约束：ROLLBACK_LOG 中的命令必须是硬编码字符串，禁止拼接外部输入（如用户输入的路径/域名），
    # 否则 eval 会执行注入代码。所有 add_rollback 调用都应使用单引号包裹字面量。
...
for _svc in nginx openresty apache2 httpd caddy; do
...
pkill -x openresty 2>/dev/null || true
...
if [ "$PORT_CONFLICT" -eq 1 ] && [ "$AUTO_YES" -eq 0 ]; then
    ...
    read -r -p "请选择 [y/n]: " CONTINUE || CONTINUE=""
    CONTINUE=${CONTINUE:-n}
...
# 无人值守模式默认不做全量系统升级，避免生产服务器因内核/依赖升级意外中断
if [ "$AUTO_YES" -eq 1 ]; then
    UPDATE_SYSTEM="n"
else
    ...
    # 区分"用户回车"（默认 y）和"EOF/非交互"（默认 n，安全起见不更新）
    if read -r -p "请选择 [y/n]: " UPDATE_SYSTEM; then
        UPDATE_SYSTEM=${UPDATE_SYSTEM:-y}  # 用户回车，默认 y
    else
        UPDATE_SYSTEM="n"  # EOF（非交互），默认 n 避免意外更新生产系统
        echo ""
        warn "非交互模式检测到，跳过系统更新"
    fi
fi
...
if ! sysctl net.ipv4.tcp_congestion_control 2>/dev/null | grep -q bbr; then
    warn "BBR 未生效（内核可能 < 4.9 或为 OpenVZ 容器），网络优化效果受限"
fi
...
# 注意：不能直接 bash -c "$(curl ...)" —— curl 失败时输出为空，bash -c "" 返回 0，
# 会被误判为安装成功，回退到最新版的逻辑永远不会触发。
# 必须先下载到临时文件，校验非空后再执行。
# 用 bash tmpfile install（不带 @）：file 成为 $0，install 成为 $1。
# install-release.sh 只用 $0 做错误消息前缀，不依赖 $0 做功能性逻辑（官方用 @ 占位也证明此点）。
XRAY_INSTALLER_TMP=$(mktemp 2>/dev/null || echo "/tmp/xray-install.$$.sh")
if curl -fsSL "$XRAY_INSTALL_URL" -o "$XRAY_INSTALLER_TMP" && [ -s "$XRAY_INSTALLER_TMP" ]; then
    if ! bash "$XRAY_INSTALLER_TMP" install --version "${XRAY_VERSION}"; then
        warn "指定版本 ${XRAY_VERSION} 安装失败，回退到最新版本..."
        bash "$XRAY_INSTALLER_TMP" install || { rm -f "$XRAY_INSTALLER_TMP"; error "Xray 安装失败，请检查网络或手动安装"; }
    fi
else
    rm -f "$XRAY_INSTALLER_TMP"
    error "无法下载 Xray 安装脚本，请检查网络连接（GitHub: ${XRAY_INSTALL_URL}）"
fi
rm -f "$XRAY_INSTALLER_TMP"
command -v xray &>/dev/null || error "Xray 安装失败，请检查网络或手动安装"
# 打印实际安装版本，便于排查（XHTTP 需 Xray ≥ v24.9，固定版本 v26.3.27 已满足）
log "已安装: $(xray version 2>/dev/null | head -1)"
...
# CN 和 SAN 首项动态取首个伪装域名，避免残留旧版硬编码
CERT_CN="${REALITY_CDNS[0]%%|*}"
SAN_LIST=""
for entry in "${REALITY_CDNS[@]}"; do
    [ -n "$SAN_LIST" ] && SAN_LIST+=","
    SAN_LIST+="DNS:${entry%%|*}"
...
if ! openssl req -new -x509 -days 3650 -key /etc/xray/server.key \
    ...
    openssl req -new -x509 -days 3650 -key /etc/xray/server.key \
        -out /etc/xray/server.crt \
        -subj "/CN=${CERT_CN}" 2>/dev/null || error "证书生成失败"
fi
...
# - fallback 用明文 UDP（1.1.1.1/8.8.8.8）：respect-rules: true 会让 DNS
#   查询的「连接方式」遵循分流规则——1.1.1.1/8.8.8.8 不命中 cncidr/GEOIP,CN
#   → 落到规则链末尾的 MATCH,Proxy → DNS 查询经代理隧道发出，明文被隧道
#   加密保护，无需 DoH 二次加密（省一次 TLS 握手，且无 dns.google 解析依赖）。
#   ⚠ 隐患：此方案依赖 MATCH,Proxy 兜底。切勿将 1.1.1.1/8.8.8.8 加入
#      DIRECT/IP-CIDR 规则，否则 DNS 查询将明文直连被 GFW 投毒，被墙域名
#      会拿到假 IP 导致代理连失败。若改规则后无法保证，请改回 DoH 更稳妥。
...
# ============================================
# 生成 VLESS 通用订阅（nodes.txt）
# 兼容旧版 v2rayN/v2rayNG/Shadowrocket 等不支持 Clash Meta YAML 的客户端
# 格式: 每行一个 vless:// 链接，用 base64 编码整文件后可被客户端作为"通用订阅"导入
# ============================================
VLESS_LINKS=""
for entry in "${REALITY_CDNS[@]}"; do
    CDN_DOMAIN="${entry%%|*}"
    ...
    # Reality 节点（Vision + Reality）
    # vless://uuid@server:port?type=tcp&security=reality&pbk=xxx&sid=xxx&sni=xxx&fp=chrome&flow=xtls-rprx-vision#name
    VLESS_LINKS+="vless://${UUID}@${SERVER_IP}:${CDN_PORT}?type=tcp&security=reality&pbk=${PUBLIC_KEY}&sid=${SHORT_ID}&sni=${CDN_DOMAIN}&fp=chrome&flow=xtls-rprx-vision#Reality-${CDN_TAG}"$'\n'

    # TLS 节点（Vision + 自签 TLS，需客户端 skip-cert-verify）
    # vless://uuid@server:8443?type=tcp&security=tls&sni=xxx&fp=chrome&allowInsecure=1&flow=xtls-rprx-vision#name
    VLESS_LINKS+="vless://${UUID}@${SERVER_IP}:8443?type=tcp&security=tls&sni=${CDN_DOMAIN}&fp=chrome&allowInsecure=1&flow=xtls-rprx-vision#TLS-${CDN_TAG}"$'\n'

    # XHTTP 节点（XHTTP + Reality）
    # vless://uuid@server:8880?type=xhttp&security=reality&path=/xhttp&host=xxx&pbk=xxx&sid=xxx&sni=xxx&fp=chrome#name
    VLESS_LINKS+="vless://${UUID}@${SERVER_IP}:8880?type=xhttp&security=reality&path=%2Fxhttp&host=${CDN_DOMAIN}&pbk=${PUBLIC_KEY}&sid=${SHORT_ID}&sni=${CDN_DOMAIN}&fp=chrome#XHTTP-${CDN_TAG}"$'\n'
done

# 写入原始 vless:// 链接文件（每行一个，部分客户端直接支持）
printf '%s' "$VLESS_LINKS" > "$WEB_ROOT/nodes.txt"
# 同时生成 base64 编码版本（通用订阅格式，更多客户端支持）
# 兼容 BusyBox base64（不支持 -w 0，但默认单行输出）和 GNU base64
# 用 || 兜底，避免 set -e 在 -w 0 失败时直接退出
printf '%s' "$VLESS_LINKS" | base64 -w 0 > "$WEB_ROOT/nodes_base64.txt" 2>/dev/null \
    || printf '%s' "$VLESS_LINKS" | base64 | tr -d '\n' > "$WEB_ROOT/nodes_base64.txt"
add_rollback "rm -f '$WEB_ROOT/nodes.txt' '$WEB_ROOT/nodes_base64.txt'"
log "VLESS 通用订阅已生成: ${WEB_ROOT}/nodes.txt"
...
# 日志函数（proxy-manager 是独立脚本，不继承 install.sh 的函数定义）
log() { echo -e "${GREEN}[INFO]${NC} $1"; }
warn() { echo -e "${YELLOW}[WARN]${NC} $1"; }
...
show_info() {
    ...
    echo ""
    echo -e "${GREEN}VLESS 通用订阅 (v2rayN/v2rayNG/Shadowrocket 旧版):${NC}"
    echo "  http://${SERVER_IP}:${SUB_PORT}/${SUB_PATH}-vless"
    echo ""
    echo -e "${YELLOW}提示: 请确保云服务器安全组已放行端口 ${SUB_PORT}${NC}"
    echo -e "${YELLOW}警告：订阅链接默认走 HTTP 明文，可能被中间人截获。${NC}"
    echo -e "${YELLOW}建议仅在可信网络使用，或通过 SFTP/SCP 直接下载 ${WEB_ROOT:-/usr/share/nginx/html}/clash.yaml 到本地。${NC}"
    echo ""
    echo -e "${BLUE}========== 节点信息 ==========${NC}"
...
case "$1" in
    ...
    rules)
        ...
        # 回退兜底：WEB_ROOT 为空或路径不存在时，尝试两个常见发行版默认路径
        [ -f "$RULES_PATH" ] || RULES_PATH="/usr/share/nginx/html/clash.yaml"
        [ -f "$RULES_PATH" ] || RULES_PATH="/var/www/html/clash.yaml"
        echo -e "${GREEN}Clash 规则文件:${NC}"
        echo "$RULES_PATH"
        echo ""
        echo -e "${YELLOW}修改后即时生效，客户端重新拉取订阅即可${NC}"
        ;;
    log)
    ...
    uninstall)
        ...
        # 删除订阅文件：优先用部署时记录的 WEB_ROOT，回退到两个常见路径兜底
        rm -f "${WEB_ROOT:-/usr/share/nginx/html}/clash.yaml" 2>/dev/null || true
        rm -f /var/www/html/clash.yaml 2>/dev/null || true
        rm -f /usr/share/nginx/html/clash.yaml 2>/dev/null || true
        # 清理 VLESS 通用订阅文件
        rm -f "${WEB_ROOT:-/usr/share/nginx/html}/nodes.txt" 2>/dev/null || true
        rm -f "${WEB_ROOT:-/usr/share/nginx/html}/nodes_base64.txt" 2>/dev/null || true
        rm -f /var/www/html/nodes.txt 2>/dev/null || true
        rm -f /var/www/html/nodes_base64.txt 2>/dev/null || true
        rm -f /usr/share/nginx/html/nodes.txt 2>/dev/null || true
        rm -f /usr/share/nginx/html/nodes_base64.txt 2>/dev/null || true
        ...
        # 卸载 Xray：先下载卸载脚本到临时文件，校验非空后执行
        # 避免直接 bash -c "$(curl)" 模式：curl 失败时 bash -c "" 返回 0，会误判为卸载成功
        # 用 bash tmpfile remove（不带 @）：file 成为 $0，remove 成为 $1
        XRAY_UNINSTALL_TMP=$(mktemp 2>/dev/null || echo "/tmp/xray-uninstall.$$.sh")
        if curl -fsSL "https://github.com/XTLS/Xray-install/raw/main/install-release.sh" -o "$XRAY_UNINSTALL_TMP" 2>/dev/null && [ -s "$XRAY_UNINSTALL_TMP" ]; then
            bash "$XRAY_UNINSTALL_TMP" remove 2>/dev/null || warn "Xray 卸载脚本执行失败，请手动清理 /usr/local/bin/xray 和 systemd 服务文件"
        else
            warn "无法下载 Xray 卸载脚本（GitHub 不可达），请手动执行: bash -c \"\$(curl -fsSL https://github.com/XTLS/Xray-install/raw/main/install-release.sh)\" @ remove"
        fi
...
if [ "$VLESS_HTTP_CODE" = "200" ]; then
    ...
    warn "订阅端点 (VLESS): HTTP ${VLESS_HTTP_CODE}（不可访问）"
    HEALTH_OK=0
fi
...
echo -e "${GREEN}VLESS 通用订阅（v2rayN/v2rayNG/Shadowrocket 旧版）:${NC}"
echo ""
echo -e "${YELLOW}  http://${SERVER_IP}:${SUB_PORT}/${SUB_PATH}-vless${NC}"
echo ""
echo -e "${RED}  警告：订阅链接默认走 HTTP 明文，可能被中间人截获。${NC}"
echo -e "${RED}  建议仅在可信网络使用，或执行以下命令把配置文件拉到本地：${NC}"
echo -e "${YELLOW}  scp root@${SERVER_IP}:${WEB_ROOT}/clash.yaml ./clash.yaml${NC}"
echo -e "${YELLOW}  scp root@${SERVER_IP}:${WEB_ROOT}/nodes.txt ./nodes.txt${NC}"
echo ""
echo -e "${GREEN}订阅文件:${NC}  ${WEB_ROOT}/clash.yaml"
echo -e "${GREEN}VLESS 文件:${NC}  ${WEB_ROOT}/nodes.txt"
echo ""
