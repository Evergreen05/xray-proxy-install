...
- **多协议节点**：Reality+Vision+Fragment（主力/防探测）、VLESS+TLS（备用）、XHTTP+Reality（CDN兼容）
- **多伪装 CDN**：5 个伪装域名（Apple 软件分发/iTunes/更新、Microsoft CDN、Bing）× 3 种网络类型 = 15 个节点，节点名标注所用 CDN；每个 Reality 入站独立端口、独立 dest
...
- 云服务器安全组放行端口：**443**、**1443**、**2443**、**3443**、**4443**、**8443**、**8880**、**10707**
...
| 7 | 生成 UUID、X25519 密钥对、Short ID、订阅路径 |
| 8 | 生成 Xray 配置文件（7 个入站：5 Reality + TLS + XHTTP）+ 配置预检 |
| 9 | 生成自签名 ECC 证书（SAN 覆盖全部伪装域名）并设置安全权限 |
| 10 | 生成 Clash 订阅配置文件（15 节点 + DNS + 分流规则） |
| 11 | 配置 Nginx 订阅端点 |
...
每个伪装 CDN 域名 × 三种网络类型（Reality / TLS / XHTTP）各组成一个节点，共 **15 个节点**。节点命名格式为 `<网络类型>-<CDN标签>`（如 `Reality-Apple-SWDIST`），导入客户端后可直接看到当前节点使用的 CDN。
...
| 网络类型 | 端口 | 协议 | 传输 | 加密 | 用途 |
|---------|------|------|------|------|------|
| Reality | 443/1443/2443/3443/4443 | VLESS | TCP + Vision | Reality | 主力推荐，防探测，Fragment 分片 |
| TLS | 8443 | VLESS | TCP + Vision | TLS（自签） | 备用节点，SNI 为对应 CDN 域名 |
| XHTTP | 8880 | VLESS | XHTTP | Reality | CDN 兼容，Reality 安全层，SNI 为对应 CDN 域名 |

- **Fragment**：TLS Client Hello 分片（100-200字节，间隔10-50ms），增强抗检测能力
- **Reality**：每个伪装域名独立入站、独立 dest，探测时返回对应 CDN 的真实证书，无需自有域名和证书
...
| 端口 | 协议 | 用途 |
|-----|------|------|
| 443 | TCP | Reality 节点 - Apple-SWDIST（swdist.apple.com） |
| 1443 | TCP | Reality 节点 - Apple-iTunes（iosapps.itunes.apple.com） |
| 2443 | TCP | Reality 节点 - Apple-Update（updates.cdn-apple.com） |
| 3443 | TCP | Reality 节点 - Microsoft-CDN（cdn-dynmedia-1.microsoft.com） |
| 4443 | TCP | Reality 节点 - Bing（www.bing.com） |
| 8443 | TCP | VLESS TLS 备用节点（5 个 CDN 共用） |
| 8880 | TCP | VLESS XHTTP CDN 节点（5 个 CDN 共用） |
...
> **注意**：
> - TLS（8443）组节点使用自签名证书，客户端需启用 **skip-cert-verify**
> - XHTTP（8880）组使用 Reality 安全层，无需 skip-cert-verify，但需较新版本 mihomo/Clash Meta 内核（≥ 2024.1）
> - Reality（443/1443/2443/3443/4443）组无需额外设置
...
- **订阅端点**：随机 16 字符十六进制路径，仅允许访问指定路径，其他路径返回 404；订阅端点关闭 access_log 保护隐私
...
- **配置预检**：jq 验证 JSON 合法性 + `xray run -test` 语义校验，不合格中止部署并回滚
