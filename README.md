...
| 10 | 生成 Clash 订阅配置文件（3 节点 + DNS + 分流规则） |
| 11 | 配置 Nginx 订阅端点 |
...
当前采用**单域名单端口 Reality**设计：默认伪装目标 `cdn-dynmedia-1.microsoft.com:443` 对应一个 Reality 入站，再搭配 TLS（8443）与 XHTTP（8880）两个备用入站，共 **3 个节点**。节点命名格式为 `<网络类型>-<CDN标签>`（如 `Reality-Microsoft-CDN`）。如需扩展，往脚本头部 `REALITY_CDNS` 数组添加 `域名|端口|标签` 即可自动级联生成 Reality/TLS/XHTTP 三组节点。
...
| 网络类型 | 端口 | 协议 | 传输 | 加密 | 用途 |
|---------|------|------|------|------|------|
| Reality | 443 | VLESS | TCP + Vision | Reality | 主力推荐，防探测，Fragment 分片 |
...
2. 主目标失败时，按序尝试备用池：`updates.cdn-apple.com` → `iosapps.itunes.apple.com` → `download-porter.hoyoverse.com` → `osxapps.itunes.apple.com` → `music.apple.com` → `tv.apple.com` → `www.mi.com` → `buylite.music.apple.com` → `www.lamer.com.hk`；
3. 备用池全部失败时，脚本会告警但**继续部署不阻断**（可能是服务器出网受限，客户端侧未必不可用）。
